...
int isthief(Vertex* pVertex, Player* pPlayer, int32_t ary[], int32_t resource_NO, int32_t number[], int32_t turn)
{
	...
	for (i = 0; i < 6; i++)
	{
		if ((pVertex[ary[i]].village != turn && pVertex[ary[i]].village > 0)|| (pVertex[ary[i]].city != turn && pVertex[ary[i]].city > 0))
		{
			amount = 1;
			break;
		}
	...
	else if (amount == 1)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			printf("Wheat: \x1b[31m%d\x1b[0m", pPlayer[i].resource[0]);
			printf(", wood: \x1b[31m%d\x1b[0m", pPlayer[i].resource[1]);
		...
		printf("wheat: \x1b[31m%d\x1b[0m", number[0]);
		printf(", wood: \x1b[31m%d\x1b[0m", number[1]);
...
void thief(Plate* pPlate, Vertex* pVertex, Player* pPlayer, int32_t turn, int32_t bank_rec[])
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (cards > 7 && i == 0)
		{
			...
			while (1)
			{
				...
				printf("wheat: \x1b[31m%d\x1b[0m", pPlayer[0].resource[0]);
				printf(", wood: \x1b[31m%d\x1b[0m", pPlayer[0].resource[1]);
				...
				printf("1: wheat, 2: wood, 3: wool, 4: ore, 5: brick\n");
	...
	pPlate[recent - 1].bandit = 0;

}

void ritter(Plate* pPlate, Vertex* pVertex, Player* pPlayer, int32_t turn, int32_t* bank_res) {
	...
	while (1) {
		...
		if (point == 1 && point != recent)
		{
			...
			a = isthief(pVertex, pPlayer, ary, pPlate[0].scene, bank_res, turn);
			pPlate[0].bandit = 1;
		}
		else if (point == 2 && point != recent)
		{
			int ary[] = { 1, 4, 5, 8, 9, 13 };
			a = isthief(pVertex, pPlayer, ary, pPlate[1].scene, bank_res, turn);
			pPlate[1].bandit = 1;
		}
		else if (point == 3 && point != recent)
		{
			int ary[] = { 2, 5, 6, 9, 10, 14 };
			a = isthief(pVertex, pPlayer, ary, pPlate[2].scene, bank_res, turn);
			pPlate[2].bandit = 1;
		}
		else if (point == 4 && point != recent)
		{
			int ary[] = { 7, 11, 12, 16, 17, 22 };
			a = isthief(pVertex, pPlayer, ary, pPlate[3].scene, bank_res, turn);
			pPlate[3].bandit = 1;
		}
		else if (point == 5 && point != recent)
		{
			int ary[] = { 8, 12, 13, 17, 18, 23 };
			a = isthief(pVertex, pPlayer, ary, pPlate[4].scene, bank_res, turn);
			pPlate[4].bandit = 1;
		}
		else if (point == 6 && point != recent)
		{
			int ary[] = { 9, 13, 14, 18, 19, 24 };
			a = isthief(pVertex, pPlayer, ary, pPlate[5].scene, bank_res, turn);
			pPlate[5].bandit = 1;
		}
		else if (point == 7 && point != recent)
		{
			int ary[] = { 10, 14, 15, 19, 20, 25 };
			a = isthief(pVertex, pPlayer, ary, pPlate[6].scene, bank_res, turn);
			pPlate[6].bandit = 1;
		}
		else if (point == 8 && point != recent)
		{
			int ary[] = { 16, 21, 22, 27, 28, 33 };
			a = isthief(pVertex, pPlayer, ary, pPlate[7].scene, bank_res, turn);
			pPlate[7].bandit = 1;
		}
		else if (point == 9 && point != recent)
		{
			int ary[] = { 17, 22, 23, 28, 29, 34 };
			a = isthief(pVertex, pPlayer, ary, pPlate[8].scene, bank_res, turn);
			pPlate[8].bandit = 1;
		}
		else if (point == 10 && point != recent)
		{
			int ary[] = { 18, 23, 24, 29, 30, 35 };
			a = isthief(pVertex, pPlayer, ary, pPlate[9].scene, bank_res, turn);
			pPlate[9].bandit = 1;
		}
		else if (point == 11 && point != recent)
		{
			int ary[] = { 19, 24, 25, 30, 31, 36 };
			a = isthief(pVertex, pPlayer, ary, pPlate[10].scene, bank_res, turn);
			pPlate[10].bandit = 1;
		}
		else if (point == 12 && point != recent)
		{
			int ary[] = { 20, 25, 26, 31, 32, 37 };
			a = isthief(pVertex, pPlayer, ary, pPlate[11].scene, bank_res, turn);
			pPlate[11].bandit = 1;
		}
		else if (point == 13 && point != recent)
		{
			int ary[] = { 28, 33, 34, 38, 39, 43 };
			a = isthief(pVertex, pPlayer, ary, pPlate[12].scene, bank_res, turn);
			pPlate[12].bandit = 1;
		}
		else if (point == 14 && point != recent)
		{
			int ary[] = { 29, 34, 35, 39, 40, 44 };
			a = isthief(pVertex, pPlayer, ary, pPlate[13].scene, bank_res, turn);
			pPlate[13].bandit = 1;
		}
		else if (point == 15 && point != recent)
		{
			int ary[] = { 30, 35, 36, 40, 41, 45 };
			a = isthief(pVertex, pPlayer, ary, pPlate[14].scene, bank_res, turn);
			pPlate[14].bandit = 1;
		}
		else if (point == 16 && point != recent)
		{
			int ary[] = { 31, 36, 37, 41, 42, 46 };
			a = isthief(pVertex, pPlayer, ary, pPlate[15].scene, bank_res, turn);
			pPlate[15].bandit = 1;
		}
		else if (point == 17 && point != recent)
		{
			int ary[] = { 39, 43, 44, 47, 48, 51 };
			a = isthief(pVertex, pPlayer, ary, pPlate[16].scene, bank_res, turn);
			pPlate[16].bandit = 1;
		}
		else if (point == 18 && point != recent)
		{
			int ary[] = { 40, 44, 45, 48, 49, 52 };
			a = isthief(pVertex, pPlayer, ary, pPlate[17].scene, bank_res, turn);
			pPlate[17].bandit = 1;
		}
		else if (point == 19 && point != recent)
		{
			int ary[] = { 41, 45, 46, 49, 50, 53 };
			a = isthief(pVertex, pPlayer, ary, pPlate[18].scene, bank_res, turn);
			pPlate[18].bandit = 1;
		}
		pPlate[recent - 1].bandit = 0;
		break;
